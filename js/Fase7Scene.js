class Fase7Scene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("boss_final", "assets/boss-final.png");
        this.load.audio("levelUp", "assets/level-up.mp3");
        this.load.image("projetil", "assets/projetil.png");
        this.load.audio("musica_boss_final", "assets/musica-boss-final.mp3");
    ...
    create() {
        ...
        // Já equipa o ESCUDO como arma secundária
        ...
        // música de fundo
        this.faseFinalMusic = this.sound.add("musica_boss_final", { loop: true, volume: 0.5 });
        this.faseFinalMusic.play();
    ...
    showGameOverScreen() {
        ...
        btn.on("pointerdown", () => {
            if (this.faseFinalMusic) this.faseFinalMusic.stop();
    ...
    startBossFight() {
        ...
        const bossX = Phaser.Math.Between(2000, 2500);
        const bossY = Phaser.Math.Between(2000, 2500);

        this.boss = this.physics.add.sprite(bossX, bossY, "boss_final")
            .setScale(0.7)
            .setCollideWorldBounds(true);

        this.boss.maxHealth = 8000;
        this.boss.health = 8000;
        this.boss.damage = 60; // << ADICIONAR ISSO AQUI
        ...
        // Barra de HP do Boss
        // Barra de HP do Boss
        this.bossHpBarBackground = this.add.rectangle(width / 2, 30, width * 0.5, 20, 0x444444)
            .setOrigin(0.5, 0)
            .setScrollFactor(0)
            .setDepth(1000);

        this.bossHpBar = this.add.rectangle(width / 2, 30, width * 0.5, 20, 0xff0000)
            .setOrigin(0.5, 0)
            .setScrollFactor(0)
            .setDepth(1001);
        ...
        this.time.addEvent({
            ...
            loop: true,
            callback: () => {
                if (!this.isGameOver && this.boss.active) {
                    this.bossShoot();
                }
            }
        });

        // Invoca minions depois de 50%
        this.minionTimer = this.time.addEvent({
            delay: 5000,
            loop: true,
            callback: () => {
                if (!this.isGameOver && this.boss.active && this.boss.health <= this.boss.maxHealth * 0.5) {
                    this.spawnMinions();
                }
            }
        });

        // Checa colisão projétil ↔ player
        this.physics.add.overlap(this.player, this.bossProjectiles, (player, proj) => {
            if (this.time.now - this.lastDamageTime < this.invulnerabilityCooldown) return;
        ...
        // Checa ataque no boss
        this.physics.add.overlap(this.enemies, this.bossProjectiles, (enemy, proj) => {
            proj.destroy();
        });
    ...
    bossShoot() {
        ...
        spreadAngles.forEach((angle) => {
            const proj = this.physics.add.sprite(this.boss.x, this.boss.y, "projetil")
                .setScale(0.6);
            ...
            proj.setVelocity(
                Math.cos(angle) * 150,  // metade da velocidade
                Math.sin(angle) * 150
            );
    ...
    spawnMinions() {
        for (let i = 0; i < 3; i++) {
            ...
            const enemy = this.enemies.create(x, y, Phaser.Math.RND.pick(["enemy1", "enemy2", "enemy3"]))
                .setScale(0.08)
                .setCollideWorldBounds(true);
    ...
    update(time, delta) {
        ...
        if (this.boss && this.bossHpBar) {
            const width = this.scale.width * 0.5 * (this.boss.health / this.boss.maxHealth);
            this.bossHpBar.width = width;
        }
        ...
        if (
            ...
            this.secondaryWeapon === "bow" &&  // ✅ só deixa ativar se for tornado
        ...
        if (this.boss && this.boss.active && !this.isPaused && !this.physics.world.isPaused) {
            const dir = new Phaser.Math.Vector2(
                this.player.x - this.boss.x,
                this.player.y - this.boss.y
            ).normalize().scale(this.bossSpeed);

            this.boss.setVelocity(dir.x, dir.y);
        }
    }
}
