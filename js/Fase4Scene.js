class Fase4Scene extends Phaser.Scene {
  ...
  create() {
    ...
    // música de fundo
    this.fase4Music = this.sound.add("musica_fase4", {
      loop: true,
      volume: 0.5,
    });
    this.fase4Music.play();

    // cenário e limites
    this.add.tileSprite(0, 0, mapSize, mapSize, "fase4_bg").setOrigin(0);
    ...
    const phaseText = this.add
      .text(width / 2, height - 280, "Fase 4 - Ira", {
  ...
  showGameOverScreen() {
    ...
    btn.on("pointerdown", () => {
      if (this.fase4Music) this.fase4Music.stop();
  ...
  checkEndOfWave() {
    if (this.enemies.getChildren().length === 0) {
      if (this.wave < this.maxWaves) {
        ...
        this.time.delayedCall(3000, () => {
          this.fase4Music.stop(); // para a música
          this.scene.stop("Fase4Scene"); // força parar a cena atual
          this.scene.start("Fase5Scene"); // vai para a próxima fase
        });
      }
    } else {
  ...
  spawnEnemyNearby() {
    ...
    const statsMap = {
      enemy1: { health: 450, speed: 160, damage: 30 },
      enemy2: { health: 600, speed: 150, damage: 40 },
      enemy3: { health: 900, speed: 110, damage: 55 },
    };
  ...
  spawnMiniBoss() {
    ...
    this.miniBoss.health = 2800; // vida aumentada
    this.miniBoss.speed = 110; // ligeiramente mais rápido
    this.miniBoss.damage = 65; // contato, se quiser
