class Fase5Scene extends Phaser.Scene {
  constructor() {
    super("Fase5Scene");
  ...
  create() {
    ...
    // música de fundo
    this.fase5Music = this.sound.add("musica_fase5", {
      loop: true,
      volume: 0.5,
    });
    this.fase5Music.play();

    // cenário e limites
    this.add.tileSprite(0, 0, mapSize, mapSize, "fase5_bg").setOrigin(0);
    ...
    const phaseText = this.add
      .text(width / 2, height - 280, "Fase 5 - Violencia", {
  ...
  showGameOverScreen() {
    ...
    btn.on("pointerdown", () => {
      if (this.fase5Music) this.fase5Music.stop();
  ...
  spawnEnemyNearby() {
    ...
    const statsMap = {
      violencia1: { health: 600, speed: 180, damage: 40, scale: 0.15 },
      violencia2: { health: 800, speed: 160, damage: 50, scale: 0.12 },
      violencia3: { health: 1100, speed: 120, damage: 70, scale: 0.1 },
    };
  ...
  spawnMiniBoss() {
    ...
    this.miniBoss = this.enemies
      .create(x, y, "violenciaBoss")
      .setScale(this.player.scaleX * 2)
      .setCollideWorldBounds(true);

    this.miniBoss.health = 3500; // vida aumentada
    this.miniBoss.speed = 120; // ligeiramente mais rápido
    this.miniBoss.damage = 80; // contato, se quiser
